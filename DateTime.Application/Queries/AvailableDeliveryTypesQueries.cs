...
Select
    T2.ВремяНаОднуПозицию * SUM(T1.КоличествоСтрок) As УсловиеКоличествоСтрок,
    Case
        When SUM(T1.Объем) < 0.8
			And SUM(T1.Вес) < 5.0 Then T2.УсловиеОбъемВес1
        When SUM(T1.Объем) < 0.8
			And SUM(T1.Вес) >= 5.0
			And SUM(T1.Вес) < 20.0 Then T2.УсловиеОбъемВес2
        When SUM(T1.Объем) < 0.8
			And SUM(T1.Вес) >= 20.0
			And SUM(T1.Вес) < 65.0 Then T2.УсловиеОбъемВес3
        When SUM(T1.Объем) < 0.8
			And SUM(T1.Вес) >= 65.0
			And SUM(T1.Вес) < 120.0 Then T2.УсловиеОбъемВес4
        When SUM(T1.Объем) < 0.8
			And SUM(T1.Вес) >= 120.0
			And SUM(T1.Вес) < 250.0 Then T2.УсловиеОбъемВес5
        When SUM(T1.Объем) < 0.8
			And SUM(T1.Вес) >= 250.0
			And SUM(T1.Вес) < 400.0 Then T2.УсловиеОбъемВес5_1
        When SUM(T1.Объем) < 0.8
			And SUM(T1.Вес) >= 400.0 Then T2.УсловиеОбъемВес5_2
        When SUM(T1.Объем) >= 0.8
			And SUM(T1.Вес) < 120.0 Then T2.УсловиеОбъемВес6
        When SUM(T1.Объем) >= 0.8
			And SUM(T1.Вес) >= 120.0
			And SUM(T1.Вес) < 250.0 Then T2.УсловиеОбъемВес7
        When SUM(T1.Объем) >= 0.8
			And SUM(T1.Вес) >= 250.0
			And SUM(T1.Вес) < 600.0 Then T2.УсловиеОбъемВес8
        When SUM(T1.Объем) >= 0.8
			And SUM(T1.Вес) >= 600.0 Then T2.УсловиеОбъемВес9
...
Group By
    ...
    T2.УсловиеОбъемВес4,
    T2.УсловиеОбъемВес5,
    T2.УсловиеОбъемВес5_1,
    T2.УсловиеОбъемВес5_2,
    T2.УсловиеОбъемВес8,
    T2.УсловиеОбъемВес9,
    T2.УсловиеОбъемВес6,
    T2.УсловиеОбъемВес7
...
Into #Temp_Remains
From
    dbo._AccumRgT21444 РезервированиеИтоги With (READCOMMITTED)
	Left Join _AccumRg21407 Резервирование With (READCOMMITTED)
...
From
    dbo._InfoRg23830 ПрогнозныеДатыПоставокНаСклады With (READCOMMITTED)
...
From
    dbo._InfoRg23830 ПрогнозныеДатыПоставокНаСклады With (READCOMMITTED{5}) 
    Inner Join SourceWarehouses 
	On ПрогнозныеДатыПоставокНаСклады._Fld23831RRef = SourceWarehouses.СкладИсточника	
Where
