...
FROM
	...
	WHERE 
		...
		LEFT JOIN dbo._InfoRg14884 СегментыНоменклатуры WITH(NOLOCK)
		ON СегментыНоменклатуры._Fld14885RRef = 0xB9B358D56E44CD1011EE94F726F8AC14
		AND МаркируемыеТовары.НоменклатураСсылка = СегментыНоменклатуры._Fld14886RRef
WHERE 
	ЕстьЗарегистрированныеШтрихкоды.Номенклатура IS NULL
	AND СегментыНоменклатуры._Fld14886RRef IS NULL
OPTION (KEEP PLAN, KEEPFIXED PLAN);
